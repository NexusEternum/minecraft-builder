...
class EventBus {
    ...
    fun register(instance: Any) {
        for (method in instance::class.java.declaredMethods) {
            ...
            list.add(Subscriber(instance, method, subscribe.priority))
    ...
    fun post(event: Any) {
        ...
        // Walk the class hierarchy to support event inheritance
        var clazz: Class<*>? = eventType
        while (clazz != null && clazz != Any::class.java) {
            subscribers[clazz]?.forEach { sub ->
                try {
                    sub.method.invoke(sub.instance, event)
                } catch (e: Exception) {
                    log.error("Error dispatching event {} to {}.{}", 
                        eventType.simpleName, sub.instance::class.simpleName, sub.method.name, e)
                }
            }
